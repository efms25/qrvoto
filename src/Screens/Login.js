      ...
      try {
        ...
        if (e.code === 'auth/user-not-found' || e.code === 'auth/wrong-password') {
          ...
          Toast.show({
            ...
            text1: 'Autenticação',
            text2:
              'Tentativas multiplas de login, aguarde um segundo para tentar se conectar novamente.',
          });
        } else {
          Toast.show({
            type: 'error',
            text1: 'Autenticação',
            text2:
              'Não foi possível realizar a autenticação no momento, tente novamente em instantes.',
          });
        }
      } finally {
        setLoadingAuth(false);
  ...
  return (
    <Center w="100%" h="100%">
      <Box safeArea p="2" py="8" w="90%" maxW="290">
        <Center my="8">
          <MCIcons name="qrcode-scan" size={60} color="#444444" />
        </Center>
        <Heading
          size="lg"
          fontWeight="600"
        ...
        <VStack space={3} mt="5">
          ...
          <Button
            ...
            mt="2"
            colorScheme="indigo"
            onPress={() => onSubmit(dataLogin.login, dataLogin.password)}
          >
            Entrar
          </Button>
        </VStack>
